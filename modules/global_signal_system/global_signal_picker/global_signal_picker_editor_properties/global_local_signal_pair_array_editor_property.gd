...
func _init(obj = null, prop = null):
	...
	recreate_drop_down_list(current_value.fake_global_local_signal_pairs)
...
func on_add_element():
	...
	current_value.fake_global_local_signal_pairs.append(GlobalLocalSignalPair.new())
...
func on_remove_element(item):
	var temp_index = 0
	for i in range(0, current_value.fake_global_local_signal_pairs.size()):
		if current_value.fake_global_local_signal_pairs[i] == item.global_local_signal_picker_pair:
			temp_index = i
	current_value.fake_global_local_signal_pairs.erase(current_value.fake_global_local_signal_pairs[temp_index])
	emit_changed(get_edited_property(), current_value)
...
func on_global_signal_item_selected(index):
	...
	for i in range(1, children.size()):
		...
		if i <= current_value.fake_global_local_signal_pairs.size():
			current_value.fake_global_local_signal_pairs[i-1].global_signal.signal_name = drop_down.selected_option
	emit_changed(get_edited_property(), current_value)
...
func on_signal_item_selected(index):
	...
	for i in range(1, children.size()):
		...
		if i <= current_value.fake_global_local_signal_pairs.size():
			current_value.fake_global_local_signal_pairs[i-1].local_signal.signal_name = drop_down.selected_option
	emit_changed(get_edited_property(), current_value)
...
func refresh_drop_down_list():
	...
	for i in range(0, children.size()):
		...
		drop_down.global_local_signal_picker_pair = current_value.fake_global_local_signal_pairs[i]
