...
func _init(obj = null, prop = null):
	...
	recreate_group_drop_down_list(current_value.fake_signal_groups)
...
func on_add_signal_group():
	...
	if updating:
		return
	current_value.fake_signal_groups.append(SignalPickerGroup.new())
	current_value.fake_signal_groups[current_value.fake_signal_groups.size() - 1].signal_to_listen = SignalPicker.new()
	refresh_group_drop_down_list()
...
func on_add_element(group, array):
	...
	for i in range(0,children.size()):
		if children[i].get_child(0) == group:
			...
			current_value.fake_signal_groups[i].signals_to_raise.append(sp)
			...
			for j in range(0 , array_children.size()):
				...
				drop_down.signal_picker = current_value.fake_signal_groups[i].signals_to_raise[j]
...
func on_item_selected(group,array,item):
	...
	for i in range(0, groups.size()):
		if groups[i].get_child(0) == group:
			for j in range(0, current_value.fake_signal_groups[i].signals_to_raise.size()):
				if current_value.fake_signal_groups[i].signals_to_raise[j] == item.signal_picker:
					current_value.fake_signal_groups[i].signals_to_raise[j].signal_name = item.selected_option
	emit_changed(get_edited_property(), current_value)
...
func on_signal_to_listen_item_selected(item):
	...
	for i in range(0, groups.size()):
		...
		if !current_value.fake_signal_groups[i].signal_to_listen:
			current_value.fake_signal_groups[i].signal_to_listen = SignalPicker.new()
		current_value.fake_signal_groups[i].signal_to_listen.signal_name = signal_to_listen.selected_option
	emit_changed(get_edited_property(), current_value)
		...
		if groups[i].get_child(0) == group:
			for j in range(0, current_value.fake_signal_groups[i].signals_to_raise.size()):
				if current_value.fake_signal_groups[i].signals_to_raise[j] == item.signal_picker:
					temp_i = i
					temp_j = j
	current_value.fake_signal_groups[temp_i].signals_to_raise.erase(current_value.fake_signal_groups[temp_i].signals_to_raise[temp_j])
	emit_changed(get_edited_property(), current_value)
	
#Synchronizes the elements and the internal property upon deletion of an element
func on_remove_group_element(item):
	var temp_index = 0
	for i in range(0, current_value.fake_signal_groups.size()):
		if current_value.fake_signal_groups[i] == item:
			temp_index = i
	current_value.fake_signal_groups.erase(current_value.fake_signal_groups[temp_index])
	emit_changed(get_edited_property(), current_value)
...
func refresh_group_drop_down_list():
	...
	for i in range(0, children.size()):
		...
		signal_picker_group_control.signal_picker_group = current_value.fake_signal_groups[i]
		select_item_by_name(signal_to_listen,current_value.fake_signal_groups[i].signal_to_listen.signal_name)
...
func recreate_group_drop_down_list(prop_groups):
	...
	for i in range(0, groups.size()):
		...
		sp_group_control.signal_picker_group = current_value.fake_signal_groups[i]
		...
		for j in range(0, items.size()):
			...
			drop_down.signal_picker = current_value.fake_signal_groups[i].signals_to_raise[j]
			select_item_by_name(drop_down,current_value.fake_signal_groups[i].signals_to_raise[j].signal_name)
